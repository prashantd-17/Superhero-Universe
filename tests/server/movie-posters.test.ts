...
test('larger catalogs keep upstream concurrency bounded', async () => {
  ...
  assert.equal(Object.keys(result.posters).length, 501);
});

test('keeps series posters instead of replacing them with landscape title cards', async () => {
  const show = { ...entry, kind: 'series' as const };
  const result = await new MoviePosterCatalog(
    [show],
    async () =>
      reply({
        query: {
          pages: [
            {
              title: entry.wikipediaTitle,
              original: { source: newImage, width: 400, height: 225 },
            },
          ],
        },
      }),
  ...
  assert.equal(result.posters[show.slug], oldImage);
  const fixed = await new MoviePosterCatalog(
    [{ ...show, posterRefresh: false }],
    async () => {
      throw new Error('A fixed poster must not be queried');
    },
    () => epoch,
  ).get();
  assert.equal(fixed.posters[show.slug], oldImage);
  assert.equal(fixed.source, 'snapshot');
});
